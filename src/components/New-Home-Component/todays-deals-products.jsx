...
function TodaysDealsProducts() {
    ...
    //const electroProducts = products.filter(product => product.tags.includes("smartphones"));
        ...
        <section className="todays-deals-products mt-[72px] mb-[72px] w-full h-full">
            <div className="container h-full min-w-[100%]">
                <div className="flex items-center justify-between w-full h-full mb-5 col-heading">
                    <h1 className="sm:text-[22px] lg:text-[28px]  font-extrabold text-headingcolor uppercase">Today’s Deals of the day</h1>
                    <button className="w-[129px] h-[48px]  bg-starColor rounded-lg text-sm font-bold text-blackcolor view-btn">VIEW ALL</button>
                </div>
                            ...
                            {electroProducts.map((product, index) => {
                                return (
                                    <SwiperSlide key={product.id}>
                                        <div className="col-product w-[300px] h-full">
                                            <Link to={`Product/${product.id}`}>
                                                <div className="content-img-product w-[300px] h-[270px] bg-bgImgProduct rounded-[26px] mb-2 flex justify-center items-center flex-col relative cursor-pointer">
